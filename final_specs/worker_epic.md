...
- Store the worker’s organization on first login.
- Deny access for workers without an invited/active status.
...
- [ ] Verify acceptance records the time of acceptance on the issue.

## UI/UX Design & User Flow
**Flow:** My Tasks -> Open Task -> Accept + ETA -> Navigate -> Start Work.

## Functional Requirements
- Accepting a task must capture the worker’s estimate (minutes/hours).
- Record the acceptance time on the issue.
- Expose ETA and elapsed time to the authority dashboard.
...
- Submit EXIF timestamp, EXIF location coordinates, and device location with the resolution evidence.
